    ...
    >
      ...
      <aside className="hidden md:flex md:w-[248px] shrink-0 flex-col border-r border-[color:var(--tc-line)] bg-[color:var(--tc-surface)]">
        <div className="px-4 pt-5 pb-4 flex items-center gap-3">
          ...
          <div className="min-w-0 flex-1">
            <p className="text-[13px] font-medium tracking-tight text-[color:var(--tc-text)] leading-none">TuComité</p>
        ...
        {restaurantName && (
          <div className="mx-3 mt-2 mb-3 flex items-center gap-2 px-3 py-2 rounded-lg bg-[color:var(--tc-panel)] border border-[color:var(--tc-line)] cursor-default">
            <span className="w-1.5 h-1.5 rounded-full bg-emerald-400 animate-pulse" />
            <span className="text-[12px] text-[color:var(--tc-text-dim)] truncate flex-1">{restaurantName}</span>
            <ChevronsUpDown className="w-3 h-3 text-[color:var(--tc-text-mute)]" />
          </div>
        )}

        <nav className="px-3 pt-1 pb-4 space-y-0.5 flex-1 overflow-y-auto">
          <p className="px-3 pt-1 pb-2 text-[10px] uppercase tracking-[0.18em] text-[color:var(--tc-text-mute)]">
            Plataforma
          </p>
          {NAV.map((item) => {
            ...
            return (
              <Link
                ...
                className={`group relative w-full flex items-center gap-2.5 px-3 py-2 rounded-lg text-[13px] transition-all duration-200 ${
                  active
                    ? "bg-[color:var(--tc-panel-strong)] text-[color:var(--tc-text)]"
        ...
        <div className="p-3 space-y-2.5">
          <SecurityCard />
          <div className="flex items-center gap-2.5 px-2 py-2 rounded-lg hover:bg-[color:var(--tc-panel)] transition-colors">
            <div className="w-8 h-8 rounded-full bg-[color:var(--tc-panel-strong)] border border-[color:var(--tc-line-strong)] flex items-center justify-center text-[11px] text-[color:var(--tc-text)] font-medium">
              {initials}
            </div>
            <div className="min-w-0 flex-1">
              <p className="text-[12.5px] text-[color:var(--tc-text)] truncate leading-tight">
                {userName || userEmail || "Usuario"}
              </p>
              <p className="text-[10.5px] text-[color:var(--tc-text-mute)] truncate leading-tight mt-0.5">
                Owner
              </p>
            </div>
            <button
              onClick={handleLogout}
              className="text-[color:var(--tc-text-mute)] hover:text-[color:var(--tc-text)] p-1 rounded transition-colors"
      ...
      <div className="flex-1 min-w-0 flex flex-col">
        <header className="h-14 flex items-center justify-between px-5 sm:px-8 border-b border-[color:var(--tc-line)] bg-[color:var(--tc-ink)]/85 backdrop-blur-xl sticky top-0 z-20">
          <div className="flex items-center gap-3 text-xs text-[color:var(--tc-text-dim)] min-w-0">
            {eyebrow ?? (
          ...
          <div className="flex items-center gap-2">
            <button
              className="hidden sm:inline-flex items-center gap-2 h-8 px-2.5 rounded-md border border-[color:var(--tc-line-strong)] bg-[color:var(--tc-panel)] text-[12px] text-[color:var(--tc-text-dim)] hover:text-[color:var(--tc-text)] hover:border-[color:var(--tc-line-strong)] transition-all"
              type="button"
            >
              <Command className="w-3.5 h-3.5" />
              <span>Buscar</span>
              <kbd className="text-[10px] text-[color:var(--tc-text-mute)] border border-[color:var(--tc-line)] rounded px-1 py-0.5 ml-2">
                ⌘K
              </kbd>
            </button>
